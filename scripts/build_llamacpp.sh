...
# 跑完直接 `bash scripts/dev_candle.sh`(网关会 spawn 常驻 llama-server 并代理 /candle/text),零手动。
#
...
cat > "$ENVFILE" <<EOF
# 由 scripts/build_llamacpp.sh 自动生成。dev_candle.sh 会自动 source 本文件。
...
echo "   启动:bash scripts/dev_candle.sh   # 网关自动 spawn 常驻 llama-server 并代理"
