请生成 2 个差异明显的歌曲结构蓝图，不要写完整歌词。
输出 JSON 字段只能包含：blueprints。
每个 blueprint 必须包含：id、mode、sections、section_roles、line_budget、energy_curve、hook_placement、bridge_function、vocal_plan、risk。
每个文本字段不超过 24 个汉字或 12 个英文词；不要写长篇理由。
