请只基于 Input JSON 中的已选曲风、structure、hook、song_brief 与 intent 摘要生成 Suno style prompt 约束，不要读取或补造未给出的上游候选方案。
...
required_sections 必须逐字复制 Input JSON.structure.sections。
...
把 Input JSON.structure.energy_curve、hook_placement、bridge_function、vocal_plan 转成可执行的 arrangement/production 描述，而不是写进歌词正文。
