...
欢迎页启动后立即渲染，不等待 sidecar、健康检查或完整曲库。单次前台等待窗口为 30 秒；瞬时 health 连接失败保持 loading 并继续轮询，超时后用户可继续等待同一个后台启动任务。`ready` 的条件是：sidecar 健康、最小歌单数据已加载、当前首曲（若存在且 playback status 为 ready）的音频 URL 已取得。没有可播放歌曲时，不能伪造 ready；应显示明确的空歌单状态。
...
sidecar 启动命令、最小歌单或首曲 URL 的真实错误进入 `failed`，欢迎页显示错误文本和重试操作。health 的瞬时网络错误不直接进入失败态；30 秒仍未 ready 时进入 `timeout`，显示“继续等待”和查看详情入口，底层轮询保持同一任务所有权。不能静默进入空音乐页。后台非关键数据失败必须在音乐页显示对应错误，不得用空数组掩盖。

## Verification

覆盖共享 Promise 去重、30 秒前台等待窗口、health 瞬时失败持续 loading、关键数据失败显式状态、只加载当前首曲 URL、gateway 不进入启动路径，以及现有音乐页生命周期和本地健康检查回归。
